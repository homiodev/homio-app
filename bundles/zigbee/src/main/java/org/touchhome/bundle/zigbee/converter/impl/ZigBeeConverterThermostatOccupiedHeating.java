...
public class ZigBeeConverterThermostatOccupiedHeating extends ZigBeeBaseChannelConverter
    ...
    public boolean initializeConverter() {
        ...
        if (attribute == null) {
            log.error("{}: Error opening device thermostat occupied heating setpoint attribute",
        ...
        try {
            if (!cluster.discoverAttributes(false).get()) {
                ...
                if (capabilities == null) {
                    log.trace("{}: Thermostat occupied heating setpoint returned null", endpoint.getIeeeAddress(), endpoint.getEndpointId());
                    return false;
                }
            } else if (!cluster.isAttributeSupported(ZclThermostatCluster.ATTR_OCCUPIEDHEATINGSETPOINT)) {
                log.trace("{}: Thermostat occupied heating setpoint not supported", endpoint.getIeeeAddress(), endpoint.getEndpointId());
                return false;
            }
        } catch (InterruptedException | ExecutionException e) {
