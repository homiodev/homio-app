...
public interface HotSpotHardwareRepository {
    ...
    @HardwareQuery(echo = "Copy required files", printOutput = true, value = "mkdir -p /etc/hostapd" +
            "cp :sysDir/hostapd.conf /etc/hostapd/hostapd.conf &&" +
            "cp :sysDir/dnsmasq.conf /etc/dnsmasq.conf &&" +
            "cp :sysDir/autohotspot.service /etc/systemd/system/autohotspot.service &&" +
            "cp :sysDir/autohotspot /usr/bin/autohotspot")
