...
public class MyNettyAuthHandler extends ChannelDuplexHandler {
    ...
    private String calcMD5Hash(String toHash) {
        try {
            ...
            ipCameraHandler.getLog().warn("NoSuchAlgorithmException error when calculating MD5 hash");
        }
        return "";
    }
    ...
    public void processAuth(String authenticate, String httpMethod, String requestURI, boolean reSend) {
        if (authenticate.contains("Basic realm=\"")) {
            ...
            ipCameraHandler.getLog().debug("Setting up the camera to use Basic Auth and resending last request with correct auth.");
        ...
        if (realm.isEmpty()) {
            ipCameraHandler.getLog().warn("Could not find a valid WWW-Authenticate response in :{}", authenticate);
            return;
        }
        ...
        if (stale.equalsIgnoreCase("true")) {
            ipCameraHandler.getLog().debug("Camera reported stale=true which normally means the NONCE has expired.");
        }
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (msg instanceof HttpResponse) {
            ...
            if (response.status().code() == 401) {
                ...
                ipCameraHandler.getLog().debug("Camera at IP:{} gave a reply with a response code of :{}",
                        ipCameraHandler.getCameraEntity().getIp(), response.status().code());
            }
        }
