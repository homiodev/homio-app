...
public class AmcrestHandler extends CameraHandler {
    ...
    public void enableLed(boolean on) {
        ...
        if (on) {
            ipCameraHandler.sendHttpGET(CM + "setConfig&Lighting[0][0].Mode=Manual");
        } else {
    ...
    public void autoLed(boolean on) {
        if (on) {
            ipCameraHandler.setChannelState(CHANNEL_ENABLE_LED, null);
    ...
    public void enableLineCrossingAlarm(boolean on) {
        if (on) {
            ipCameraHandler.sendHttpGET(CM + "setConfig&VideoAnalyseRule[0][1].Enable=true");
        } else {
            ipCameraHandler.sendHttpGET(CM + "setConfig&VideoAnalyseRule[0][1].Enable=false");
        }
    }
    ...
    public void activateAlarmOutput(boolean on) {
        if (on) {
            ipCameraHandler.sendHttpGET(CM + "setConfig&AlarmOut[0].Mode=1");
        } else {
            ipCameraHandler.sendHttpGET(CM + "setConfig&AlarmOut[0].Mode=0");
        }
    }
    ...
    public void activateAlarmOutput2(boolean on) {
        if (on) {
            ipCameraHandler.sendHttpGET(CM + "setConfig&AlarmOut[1].Mode=1");
        } else {
            ipCameraHandler.sendHttpGET(CM + "setConfig&AlarmOut[1].Mode=0");
        }
    }
    ...
    /*public List<CameraAction> getCameraActions() {
        return CameraAction.builder()
                ...
                }, () -> ipCameraHandler.sendHttpGET(CM + "getConfig&name=CrossLineDetection[0]"))
                ...
                .add(CHANNEL_ENABLE_PRIVACY_MODE, UIFieldType.Boolean, param -> {
                    if (!param.getBoolean("value")) {
                        ipCameraHandler.sendHttpGET(CM + "setConfig&LeLensMask[0].Enable=false");
                    } else {
                        ipCameraHandler.sendHttpGET(CM + "setConfig&LeLensMask[0].Enable=true");
                    }
