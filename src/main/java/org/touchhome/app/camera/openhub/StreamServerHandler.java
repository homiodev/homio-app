...
import org.touchhome.app.camera.openhub.type.InstarHandler;
...
/**
 * responsible for handling streams and sending any requested files to openHABs features.
 */
@Log4j2
public class StreamServerHandler extends ChannelInboundHandlerAdapter {
    ...
    public StreamServerHandler(IpCameraHandler ipCameraHandler) {
        ...
        this.whiteList = ipCameraHandler.getWhiteList();
    }
        ...
        try {
            if (msg instanceof HttpRequest) {
                ...
                if ("GET".equalsIgnoreCase(httpRequest.method().toString())) {
                    log.debug("Stream Server recieved request \tGET:{}", httpRequest.uri());
                    ...
                    switch (queryStringDecoder.path()) {
                        ...
                        case "/ipcamera.gif":
                            ...
                            return;
                        case "/ipcamera.jpg":
                            if (!ipCameraHandler.snapshotPolling && !ipCameraHandler.snapshotUri.equals("")) {
                        ...
                        case "/instar":
                            ...
                            instar.alarmTriggered(httpRequest.uri());
                            ctx.close();
                            return;
                        case "/ipcamera0.ts":
                        default:
                    ...
                    switch (httpRequest.uri()) {
                        ...
                        case "/OnvifEvent":
                            ...
                            break;
                        default:
                            log.debug("Stream Server recieved unknown request \tPOST:{}", httpRequest.uri());
                            break;
                    }
                }
            }
    ...
    private void sendFile(ChannelHandlerContext ctx, String fileUri, String contentType) throws IOException {
        Path file = ipCameraHandler.getFfmpegOutputPath().resolve(fileUri);
