...
public class Completion implements Comparable<Completion> {
  ...
  String filterText;
  /**
   * Select this item when showing. *Note* that only one completion item can be selected and that the editor decides which item that is. The rule is that the *key* item of those
  ...
  IRange range;
  /**
   * An optional set of characters that when pressed while this completion is active will accept it key and then type that character. *Note* that all commit characters should
   * have
