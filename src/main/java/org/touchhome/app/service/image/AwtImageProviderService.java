package org.touchhome.app.service.image;
...
public class AwtImageProviderService implements ImageProviderService {
    ...
    @Override
    public byte[] overlayImage(byte[] bgImage, byte[] fgImage, int x, int y, int width, int height, String formatType) {
        return handleChangeImage(bgImage, formatType, bgBufferedImage -> {
            ...
            g.drawImage(fgBufferedImage, x, y,
                    ...
                    height <= 0 ? fgBufferedImage.getHeight() : height, null);
            g.dispose();
    ...
    public byte[] cropImage(byte[] image, int x, int y, int width, int height, String formatType) {
        return handleChangeImage(image, formatType, bufferedImage -> bufferedImage.getSubimage(x, y, width, height));
    }
    ...
    private BufferedImage createBufferedImage(int w, int h, BufferedImage bufferedImage, String formatType) {
        if (bufferedImage.getType() == BufferedImage.TYPE_BYTE_INDEXED) {
    ...
    public byte[] resizeImage(byte[] image, int width, int height, String formatType) {
        return handleChangeImage(image, formatType, bufferedImage -> {
            int targetWidth = width <= 0 ? bufferedImage.getWidth() * bufferedImage.getHeight() / height : width;
            int targetHeight = height <= 0 ? bufferedImage.getHeight() * bufferedImage.getWidth() / width : height;

            BufferedImage resizedImage = createBufferedImage(targetWidth, targetHeight, bufferedImage, formatType);
    ...
    public byte[] rotateImage(byte[] image, int angle, String formatType) {
        ...
        return handleChangeImage(image, formatType, bufferedImage -> {
            ...
            double w = Math.floor(bufferedImage.getWidth() * cos + bufferedImage.getHeight() * sin);
            double h = Math.floor(bufferedImage.getHeight() * cos + bufferedImage.getWidth() * sin);
            BufferedImage rotatedImage = createBufferedImage((int) w, (int) h, bufferedImage, formatType);
            AffineTransform at = new AffineTransform();
    ...
    public byte[] translateImage(byte[] image, float tx, float ty, String formatType) {
        return handleChangeImage(image, formatType, bufferedImage -> {
            ...
            double sin = Math.abs(Math.sin(rads));
            double cos = Math.abs(Math.cos(rads));
            int w = (int) Math.floor(bufferedImage.getWidth() * cos + bufferedImage.getHeight() * sin);
            int h = (int) Math.floor(bufferedImage.getHeight() * cos + bufferedImage.getWidth() * sin);
            BufferedImage rotatedImage = createBufferedImage(w, h, bufferedImage, formatType);
            AffineTransform at = new AffineTransform();
            at.translate(tx, ty);
            return rotatedImage;
        });
    ...
    @Override
    @SneakyThrows
    public byte[] addText(byte[] image, StylePosition stylePosition, String color, String text, String formatType) {
        return handleImage(image, formatType, (g, bufferedImage) -> {
            g.setFont(new Font("TimesRoman", Font.BOLD, 30));
            g.setColor(Color.decode(color));
            Pair<Integer, Integer> position = getTextPosition(g, bufferedImage, text, stylePosition);
            if (position != null) {
