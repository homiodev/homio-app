...
public class JavaScriptBuilderImpl implements JavaScriptBuilder {
    ...
    public String build() {
        ...
        if (!css.isEmpty()) {
            ((JsMethodImpl) this.readyOnClient()).addChild(() -> "this.addGlobalStyle('" + String.join(" ", css) + "')", 1);
        }
        ...
        if (!jsServerVariables.variables.isEmpty()) {
            ...
            readVariablesOnServerKeys.raw(() -> "return [" + String.join(" \", \"", jsServerVariables.variables.keySet()) + "];");
   ...
   /* @Override
    ...
    public JsMethodImpl js(String methodName, String... params) {
        ...
        this.jsMethods.computeIfAbsent(methodName, s -> new JsMethodImpl(methodName, params, false, 0));
    ...
    public JsMethod readyOnClient() {
        this.jsMethods.computeIfAbsent("readyOnClient", s -> new JsMethodImpl("readyOnClient", new String[0], false, 0));
    ...
    public class JSInputImpl implements JSInput {
        ...
        private JSInputImpl(String input, String inputContent, int level, JSInput parent, Consumer<JSStyle> jsStyleContext) {
   ...
   /* public class JSList {
    ...
    public class JsMethodImpl extends JSCodeContextImpl<JsMethod> implements JsMethod {
        ...
        JsMethodImpl(String methodName, String[] params, boolean ngFunc, int level) {
            ...
            this.ngFunc = ngFunc;
            if (ngFunc) {
                this.addChildNoTab(new SBuilder("this." + methodName + " = function(" + String.join(",", params) + ") {"));
            } else {
                this.addChildNoTab(() -> "function " + methodName + "(" + String.join(",", params) + ") {");
            }
        }
    ...
    public class JSCodeContextImpl<T> extends CommonBuilder implements JSCodeContext<T> {
        ...
        @Override
        public void cond(Consumer<JsCond> jsCondConsumerContext, Consumer<JsCondBody> methodContext) {
