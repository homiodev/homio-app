...
public class Scratch3DataBlocks extends Scratch3ExtensionBlocks {
    ...
    public Scratch3DataBlocks(EntityContext entityContext, BroadcastLockManagerImpl broadcastLockManager) {
    ...
    private void onChangeVariableHat(WorkspaceBlock workspaceBlock) {
        workspaceBlock.handleNext(next -> {
            ...
            BroadcastLock lock = broadcastLockManager.getOrCreateLock(workspaceBlock, variableId);
    ...
    private Object groupVariableReporter(WorkspaceBlock workspaceBlock) {
        String groupVariablesItem = workspaceBlock.getFieldId("group_variables_group");
        return groupVariablesItem == null ? null : entityContext.var().get(groupVariablesItem);
    }
    ...
    private enum WhenValueOperator {
        ...
        public static WhenValueOperator getByOp(String operator) {
            for (WhenValueOperator item : WhenValueOperator.values()) {
                if (item.op.equals(operator)) return item;
            }
