...
public class Scratch3AudioBlocks extends Scratch3ExtensionBlocks {
  ...
  public Scratch3AudioBlocks(EntityContext entityContext, AudioService audioService) {
    ...
    blockCommand(10, "play", "Play file [FILE] to [SINK]|Volume [VOLUME]", this::playFileCommand, block -> {
      block.addArgument("FILE", this.audioMenu);
      block.addArgument("SINK", this.sinkMenu);
      block.addArgument("VOLUME", 100);
    });

    blockCommand(12, "play_part", "Play file [FILE] to [SINK]|Volume [VOLUME], From [FROM]sec. Length [LENGTH]sec.",
        this::playPartFileCommand, block -> {
          block.addArgument("FILE", this.audioMenu);
          block.addArgument("SINK", this.sinkMenu);
          block.addArgument("VOLUME", 100);
    ...
    blockCommand(20, "stop", "Stop [SINK]", this::stopCommand, block -> {
      block.addArgument("SINK", this.sinkMenu);
    });

    blockCommand(30, "resume", "Resume [SINK]", this::resumeCommand, block -> {
      block.addArgument("SINK", this.sinkMenu);
    });

    blockReporter(100, "info", "[FILE] [VALUE]", this::getInfoReporter, block -> {
      block.addArgument("FILE", this.audioMenu);
      block.addArgument(VALUE, this.infoMenu);
    });

    blockReporter(130, "tts", "text [VALUE] to audio [TTS]", this::getTextToAudioReporter, block -> {
  ...
  private State getInfoReporter(WorkspaceBlock workspaceBlock) throws Exception {
    return handleFile(workspaceBlock, file ->
        workspaceBlock.getMenuValue(VALUE, this.infoMenu).handler.apply(file));
  }
  ...
  private enum AudioInfo {
    ...
    BitRate(file -> new DecimalType(AudioFileIO.read(file).getAudioHeader().getBitRateAsNumber())),
    SampleRate(file -> new DecimalType(AudioFileIO.read(file).getAudioHeader().getSampleRateAsNumber())),
