...
public class HomioApplication {

  static {
    System.setProperty(
        "java.util.logging.SimpleFormatter.format",
        "[%1$tY-%1$tm-%1$td %1$tH:%1$tM:%1$tS] %4$-5s - %5$s%n");
  }
  ...
  public static void main(String[] args) throws IOException {
    ...
    if(!isAddOpensPresent()) {
      throw new IllegalStateException("Add-opens is not present");
    }
    ...
    try {
      ...
      if (!context.isRunning()) {
        ...
        Files.deleteIfExists(
            Paths.get(System.getProperty("rootPath")).resolve("homio-app.jar_backup"));
  ...
  private static String getAppVersion() throws IOException {
    ...
    try (InputStream inputStream =
        HomioApplication.class.getResourceAsStream("/META-INF/MANIFEST.MF")) {
      if (inputStream != null) {
    ...
    if (StringUtils.isEmpty(sysRootPath)) {
      rootPath =
          (SystemUtils.IS_OS_WINDOWS
              ? SystemUtils.getUserHome().toPath().resolve("homio")
              : Paths.get("/opt/homio"));
    } else {
  ...
  private static boolean isAddOpensPresent() {
    return java.lang.ModuleLayer.boot()
            .findModule("java.base")
            .map(m -> {
              try {
                return m.isOpen("java.net", HomioApplication.class.getModule());
              } catch (Exception e) {
                return false;
              }
            })
