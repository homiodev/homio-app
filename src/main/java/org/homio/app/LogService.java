...
public class LogService
  ...
  private static final org.apache.logging.log4j.Logger maverickLogger =
      LogManager.getLogger("com.ssh.maverick");
  ...
  Set.of(
          ...
          "com.zaxxer",
          "org.hibernate");*/
  private static final BlockingQueue<LogWrapper> eventQueue = new LinkedBlockingQueue<>();

  static {
    try {
      Log.setDefaultContext(
          new DefaultLoggerContext() {
            ...
            public synchronized void log(Log.Level level, String msg, Throwable e, Object... args) {
              maverickLogger.log(
                  getLogLevel(level), DefaultLoggerContext.prepareLog(level, msg, e, args));
            }
    ...
    new Thread(
            () -> {
              while (true) {
                try {
                  globalAppender.logStrategy.accept(eventQueue.take());
                } catch (Exception ex) {
    ...
    if (loggerContext.getExternalContext() != null) {
      ...
      for (String appenderName : appenderNames) {
        ...
        if (appender instanceof RollingFileAppender) {
          var defineAppender =
              new DefinedAppenderConsumer(
                  fetchAppenderPrefixSet(configuration, appenderName),
      ...
      MessageFactory messageFactory =
          CommonUtils.newInstance(AbstractLogger.DEFAULT_MESSAGE_FACTORY_CLASS);
          ...
          new AbstractFilter() {
            public void logIfRequire(
              ...
              if (level.intLevel() == Level.DEBUG.intLevel()
                  && disabledDebugPackage(logger.getName())) {
                return;
              }
              if (level.intLevel() <= Level.DEBUG.intLevel()) {
                Message data =
                    params == null || params.length == 0
                        ...
                        : messageFactory.newMessage(message, params);
                LogEvent event =
                    log4jLogEventFactory.createEvent(
                        logger.getName(), marker, FQCN, level, data, null, data.getThrowable());
                eventQueue.add(new LogWrapper(event, true));
              }
            }
        ...
        .addEntityUpdateListener(
            ...
            baseEntity -> {
              if (baseEntity instanceof HasEntityLog
                  && globalAppender.logConsumers.containsKey(baseEntity.getEntityID())) {
                globalAppender.logConsumers.get(baseEntity.getEntityID()).debug =
                    ((HasEntityLog) baseEntity).isDebug();
              }
            });
    context
        .event()
  ...
  private static Path createLogFile(@NotNull BaseEntity entity, @NotNull String suffix) {
    ...
    try {
      ...
      System.out.printf(
          "Error during truncate file: %s. Error: %s%n", path, CommonUtils.getErrorMessage(ex));
    }
    return file;
  }
  ...
  private static void sendLogEvent(LogEvent event, ThrowingConsumer<String, Exception> consumer) {
    try {
      boolean entityPrefix =
          Optional.ofNullable(event.getMessage().getFormat())
              .map(s -> s.startsWith("[{}]: "))
              .orElse(false);
  ...
  private static Set<String> fetchAppenderPrefixSet(
      Configuration configuration, String appenderName) {
    ...
    if (entityLoggers != null) {
      for (FileLoggerImpl fileLogger : entityLoggers.values()) {
        try {
          ...
          log.error(
              "Unable to delete entity file log handler: {}", CommonUtils.getErrorMessage(ex));
        }
      }
    }
  }
  ...
  public FileLogger getFileLogger(BaseEntity baseEntity, String suffix) {
    return fileLoggers
        .computeIfAbsent(baseEntity.getEntityID(), s -> new ConcurrentHashMap<>())
  ...
  public static class GlobalAppender extends CountingNoOpAppender {
    ...
    protected Consumer<LogWrapper> logStrategy =
        event -> {
    ...
    public void append(LogEvent event) {
      this.logStrategy.accept(new LogWrapper(event, false));
    }

    // flush buffered log events and clean buffer
    void flushBufferedLogs() {
      for (LogWrapper bufferedLogEvent : bufferedLogEvents) {
    ...
    private void sendLogs(Context context, LogWrapper wrapper) {
      ...
      if (wrapper.handleLogConsumers) {
        for (LogConsumer logConsumer : logConsumers.values()) {
          ...
          if (logConsumer.logTopics.stream().anyMatch(l -> l.test(event))) {
            sendLogEvent(event, message -> writeEvent(context, logConsumer, message));
          }
        }
      }
    }

    private static void writeEvent(Context context, LogConsumer logConsumer, String message)
        throws IOException {
      Files.writeString(
          logConsumer.logFile, message + System.lineSeparator(), StandardOpenOption.APPEND);
  ...
  private record EntityLogBuilderImpl(BaseEntity entity, LogConsumer logConsumer)
      implements EntityLogBuilder {

    @Override
    @SneakyThrows
    public void addTopic(@NotNull String topic, String filterByField) {
      String filterValue = getFilterValue(filterByField);
      logConsumer.logTopics.add(
          filterValue == null
              ? logEvent -> logEvent.getLoggerName().startsWith(topic)
              : logEvent ->
                  logEvent.getLoggerName().startsWith(topic)
                      && logEvent.getMessage().getFormattedMessage().contains(filterValue));
    }

    private @Nullable String getFilterValue(String filterByField)
        throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
      return isEmpty(filterByField)
          ? null
          : String.valueOf(
              MethodUtils.invokeMethod(entity, "get" + StringUtils.capitalize(filterByField)));
    }
  }
  ...
  private record LogWrapper(LogEvent event, boolean handleLogConsumers) {}
}
