...
public class RouteController {
  ...
  public RouteController(
      ...
      Context context) {
    this.uiSidebarMenuClasses = classFinder.getClassesWithAnnotation(UIRouteMenu.class);
  ...
  private static @NotNull HttpHeaders buildResponseHeader(
      ResponseEntity<byte[]> response, Map<String, String> applyHeader) {
  ...
  private static byte[] modifyResponseBody(byte[] body, String baseUrl, String redirectURI) {
    ...
    doc.head().prependElement("base").attr("href", baseUrl + "/");
  ...
  private static void modifyRequestBody(
      @NotNull ProxyExchange<byte[]> proxy, @NotNull HttpServletRequest request)
  ...
  private static void modifyHeaders(
      @NotNull HttpServletRequest request,
      ProxyExchange<byte[]> proxyExchange,
      URI uri,
      RouteProxyImpl routeProxy) {
  ...
  public ResponseEntity<?> proxyPut(
      @PathVariable("entityID") String entityID,
      HttpServletRequest request,
      ProxyExchange<byte[]> proxy) {
    return proxyUrl(proxy, request, entityID, ProxyExchange::post);
  ...
  public ResponseEntity<?> proxyDelete(
      @PathVariable("entityID") String entityID,
      HttpServletRequest request,
      ProxyExchange<byte[]> proxy) {
  ...
  private ResponseEntity<?> proxyUrl(
    ...
    uri =
        new URI(uri.getScheme(), uri.getAuthority(), uri.getPath(), request.getQueryString(), null);
    ...
    if (response.getStatusCode().is3xxRedirection()) {
      ...
      String redirectPath =
          locationURI == null || "./".equals(locationURI.getPath()) ? "" : locationURI.getPath();
    ...
    return ResponseEntity.status(response.getStatusCode()).headers(responseHeaders).body(body);
  }
  ...
  private Map<String, List<SidebarMenuItem>> getMenu() {
    ...
    for (Class<?> item : this.uiSidebarMenuClasses) {
      UIRouteMenu mainRoutes = item.getDeclaredAnnotation(UIRouteMenu.class);
      if (mainRoutes != null) {
        getSubMenu(sidebarMenus, item);
      }
    }
  ...
  private void getSubMenu(Map<String, List<SidebarMenuItem>> sidebarMenus, Class<?> item) {
    var route = CommonUtils.getClassRoute(item);
    if (route == null) {
      throw new ServerException("No route found for class: " + item.getName());
    }
    String parent = route.parent().name().toLowerCase();
    if (!sidebarMenus.containsKey(parent)) {
      sidebarMenus.put(parent, new ArrayList<>());
    }
    sidebarMenus.get(parent).add(SidebarMenuItem.fromAnnotation(item, route));
  }

  private List<RouteDTO> getRoutes() {
    List<RouteDTO> routes = new ArrayList<>();
    for (Class<?> aClass : this.uiSidebarMenuClasses) {
      UIRouteMenu mainRoutes = aClass.getDeclaredAnnotation(UIRouteMenu.class);
      if (mainRoutes != null) {
        addRouteFromUISideBarMenu(routes, aClass);
      }
    }
  ...
  private void addRouteFromUISideBarMenu(List<RouteDTO> routes, Class<?> aClass) {
    var entityRoute = CommonUtils.getClassRoute(aClass);
    if (entityRoute == null) {
      throw new ServerException("No route found for class: " + aClass.getSimpleName());
    }
    String href = StringUtils.defaultIfEmpty(entityRoute.path(), aClass.getSimpleName());
    RouteDTO route = new RouteDTO(entityRoute.parent().name().toLowerCase() + "/" + href);
    route.path = href;
    route.allowCreateNewItems = entityRoute.allowCreateNewItems();
    route.sort =
        Stream.of(entityRoute.sort()).filter(s -> !s.isEmpty()).collect(Collectors.toList());
    route.filter =
        Stream.of(entityRoute.filter()).filter(s -> !s.isEmpty()).collect(Collectors.toList());
    routes.add(route);
  }
  ...
  public record SidebarMenuItem(String href, String icon, String bg, String label, int order) {
    ...
    static SidebarMenuItem fromAnnotation(Class<?> clazz, CommonUtils.Route route) {
      return new SidebarMenuItem(
          StringUtils.defaultIfEmpty(route.path(), clazz.getSimpleName()),
          route.icon(),
          route.color(),
          clazz.getSimpleName(),
          route.order());
