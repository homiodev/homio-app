...
public class ContextHardwareImpl implements ContextHardware {
  ...
  private static JsonNode getFoundAsset(
      Map<String, JsonNode> assetNames, Function<String, Boolean> filter) {
    return assetNames.entrySet().stream()
  ...
  public static @NotNull Architecture getArchitecture(@NotNull Context context) {
    ...
    String architecture =
        context.getBean(MachineHardwareRepository.class).getMachineInfo().getArchitecture();
  ...
  public void onContextCreated() throws Exception {
    ...
    HardwareUtils.APP_ID = context.setting().getEnv("app-id", uuid, true);
    MACHINE_IP_ADDRESS = networkHardwareRepository.getIPAddress();
    HardwareUtils.RUN_COUNT = context.setting().getEnv("run-count", 1, true);
    context.setting().setEnv("run-count", HardwareUtils.RUN_COUNT + 1);
  }
  ...
  @Override
  public @NotNull String executeNoErrorThrow(
      @NotNull String command, int maxSecondsTimeout, @Nullable ProgressBar progressBar) {
    return hardwareRepository.executeNoErrorThrow(command, maxSecondsTimeout, progressBar);
  }

  @Override
  public @NotNull List<String> executeNoErrorThrowList(
      @NotNull String command, int maxSecondsTimeout, @Nullable ProgressBar progressBar) {
  ...
  public @NotNull String execute(
      @NotNull String command, int maxSecondsTimeout, ProgressBar progressBar) {
  ...
  @Override
  public @NotNull ContextHardware installSoftware(
      @NotNull String soft, int maxSecondsTimeout, ProgressBar progressBar) {
    hardwareRepository.installSoftware(soft, maxSecondsTimeout, progressBar);
    return this;
  }

  @Override
  public @NotNull ContextHardware uninstallSoftware(
      @NotNull String soft, int maxSecondsTimeout, @Nullable ProgressBar progressBar) {
  ...
  public @NotNull ProcessStat getProcessStat(long pid) {
    if (SystemUtils.IS_OS_WINDOWS) {
      String result =
          execute(
              "powershell -Command \"(Get-Process -Id "
                  + pid
                  + " | Select-Object WS, CPU) | ConvertTo-Json\"");
      JsonNode jsonNode;
      ...
      String[] result =
          execute("ps -p " + pid + " -o %cpu,%mem,rss --no-headers").trim().split(" ");
  ...
  public @NotNull JsonNode findAssetByArchitecture(JsonNode release) {
    ...
    List<Entry<String, JsonNode>> sortedAssets =
        assetNames.entrySet().stream()
    ...
    if (!sortedAssets.isEmpty()) {
      return sortedAssets.getLast().getValue();
    }
    ...
    throw new IllegalStateException(
        "Unable to find release asset for current architecture: "
            + architecture.name()
            + ". Available assets:\n\t"
            + String.join("\n\t", assetNames.keySet()));
  }
  ...
  public String getServerUrl() {
    ...
    boolean cloudOnline =
        context.getBean(CloudService.class).getCurrentEntity().getStatus().isOnline();
    if (cloudOnline) {
