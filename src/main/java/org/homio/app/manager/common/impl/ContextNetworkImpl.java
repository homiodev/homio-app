...
public class ContextNetworkImpl implements ContextNetwork {
  ...
  private final Map<String, BiConsumer<List<CidrAddress>, List<CidrAddress>>>
      networkAddressChangeListeners = new HashMap<>();
  private final Map<String, UdpContext> listenUdpMap = new HashMap<>();

  @Getter private final Map<String, HttpRequests> scratchHttpRequests = new ConcurrentHashMap<>();
  ...
  @Override
  public void addNetworkAddressChanged(
      @NotNull String key, @NotNull BiConsumer<List<CidrAddress>, List<CidrAddress>> listener) {
  ...
  public void onContextCreated() {
    ...
    context
        .bgp()
        .builder("network-interface-poll")
        .delay(Duration.ofSeconds(1))
        .interval(Duration.ofSeconds(60))
        .execute(this::pollAndNotifyNetworkInterfaceAddress);
  }

  private void pollAndNotifyNetworkInterfaceAddress() {
    fetchIpAddress();

    Collection<CidrAddress> newInterfaceAddresses =
        NetworkHardwareRepository.getAllInterfaceAddresses();
    if (lastKnownInterfaceAddresses == null) {
      lastKnownInterfaceAddresses = newInterfaceAddresses;
      return;
    }
    // Look for added addresses to notify
    List<CidrAddress> added =
        newInterfaceAddresses.stream()
            .filter(newInterfaceAddr -> !lastKnownInterfaceAddresses.contains(newInterfaceAddr))
            .toList();

    // Look for removed addresses to notify
    List<CidrAddress> removed =
        lastKnownInterfaceAddresses.stream()
            .filter(
                lastKnownInterfaceAddr -> !newInterfaceAddresses.contains(lastKnownInterfaceAddr))
            .toList();
    ...
    if (!added.isEmpty() || !removed.isEmpty()) {
      if (log.isDebugEnabled()) {
        log.debug(
            "added {} network interfaces: {}", added.size(), Arrays.deepToString(added.toArray()));
        log.debug(
  ...
  private void fetchIpAddress() {
    ...
    if (SystemUtils.IS_OS_LINUX) {
      if (MACHINE_IP_ADDRESS.startsWith("169.254")) {
        try {
          log.warn(
              "Device ip address is: {}. Trying to call autohotspot",
              machineHardwareRepository.execute("/usr/bin/autohotspot", 60));
        } catch (Exception ex) {
          log.warn(
              "Device ip address is: {}. Unable to run autohotspot to fix: {}",
              MACHINE_IP_ADDRESS,
  ...
  private void notifyUpAddressChangeListeners(List<CidrAddress> added, List<CidrAddress> removed) {
    ...
    context
        ...
        .execute(
            () -> {
              for (BiConsumer<List<CidrAddress>, List<CidrAddress>> listener :
                  networkAddressChangeListeners.values()) {
  ...
  public void listenUdp(
      ...
      String host,
      int port,
      @NotNull BiConsumer<DatagramPacket, String> listener) {
    ...
    if (!this.listenUdpMap.containsKey(hostPortKey)) {
      ...
      try {
        ...
        scheduleFuture =
            context
                .bgp()
                .builder("listen-udp-" + hostPortKey)
                .execute(
                    () -> {
        ...
        context
            ...
            .addOrUpdateBlock(
                "UPD",
                "UDP",
                new Icon("fas fa-kip-sign", "#482594"),
                blockBuilder -> {
                  String info =
                      Lang.getServerMessage(
                          "UDP_ERROR", Map.of("key", hostPortKey, "msg", ex.getMessage()));
  ...
  @Override
  public void stopListenUdp(@NotNull String key) {
  ...
  public void addHttpRequest(
      @NotNull String path,
      @NotNull Lock lock,
      @Nullable AtomicReference<CompletableFuture<String>> answerLock) {
    scratchHttpRequests.put(path, new HttpRequests(lock, answerLock));
  }

  public void removeHttpRequest(@NotNull String path) {
    scratchHttpRequests.remove(path);
  }
  ...
  public record HttpRequests(Lock lock, AtomicReference<CompletableFuture<String>> answerLockRef) {}
}
