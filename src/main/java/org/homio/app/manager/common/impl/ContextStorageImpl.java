...
public class ContextStorageImpl implements ContextStorage {
    ...
    private final DeviceSeriesRepository deviceSeriesRepository;
    ...
    public <T extends BaseEntity> @NotNull T save(@NotNull T entity, boolean fireNotifyListeners) {
        ...
        if (entity.getCreationTime() != null && entity.isDisableEdit() && !context.user().isSuperAdmin()) {
            // eventually allow to edit if it's primary user
            if (user == null || !"dvc_user-admin_primary".equals(user.getEntityID())) {
                throw new IllegalAccessException("User is unable to update entity");
            }
        }
    ...
    private <T extends BaseEntity> List<T> findAllByRepository(Class<BaseEntity> clazz) {
        ...
        if (Modifier.isAbstract(clazz.getModifiers())) {
            ...
            } else if (DeviceSeriesEntity.class.isAssignableFrom(clazz)) {
                repository = deviceSeriesRepository;
            } else {
