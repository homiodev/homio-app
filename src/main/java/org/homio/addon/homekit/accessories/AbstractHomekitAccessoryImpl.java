...
import org.homio.addon.homekit.HomekitService;
...
public abstract class AbstractHomekitAccessoryImpl implements HomekitAccessory {
    ...
    private final Map<Class<? extends Characteristic>, Characteristic> rawCharacteristics =
            new HashMap<>();
    // private List<HomekitTaggedItem> characteristics;
    // @Getter
    //private  HomekitTaggedItem accessory;
     ...
     */
    public AbstractHomekitAccessoryImpl(HomekitEndpointEntity endpoint) {
        this.endpoint = endpoint;
        // initialize the AccessoryInformation Service with defaults if not specified
        rawCharacteristics.put(
                NameCharacteristic.class,
                new NameCharacteristic(
                        () -> {
                            return CompletableFuture.completedFuture(endpoint.getDevice().getTitle());
                        }));
        ...
        if (!isLinkedService()) {
            ...
            if (!rawCharacteristics.containsKey(ManufacturerCharacteristic.class)) {
                rawCharacteristics.put(
                        ManufacturerCharacteristic.class,
            ...
            if (!rawCharacteristics.containsKey(SerialNumberCharacteristic.class)) {
                rawCharacteristics.put(
                        SerialNumberCharacteristic.class,
                        new SerialNumberCharacteristic(
                                () -> CompletableFuture.completedFuture("serian-number")));
            }
            if (!rawCharacteristics.containsKey(FirmwareRevisionCharacteristic.class)) {
                rawCharacteristics.put(
                        FirmwareRevisionCharacteristic.class,
                        new FirmwareRevisionCharacteristic(
                                () -> CompletableFuture.completedFuture("firmware")));
            }
  ...
  /*protected void subscribe(
      HomekitCharacteristicType characteristicType, HomekitCharacteristicChangeCallback callback) {
    final Optional<HomekitTaggedItem> characteristic = getCharacteristic(characteristicType);
    if (characteristic.isPresent()) {
      getUpdater().subscribe(characteristic.get(), characteristicType.getTag(), callback);
    } else {
  ...
  /*protected void unsubscribe(HomekitCharacteristicType characteristicType) {
    final Optional<HomekitTaggedItem> characteristic = getCharacteristic(characteristicType);
    if (characteristic.isPresent()) {
      getUpdater().unsubscribe(characteristic.get(), characteristicType.getTag());
    } else {
      logger.warn("Missing mandatory characteristic {}", characteristicType);
    }
  }*/
  ...
  /*protected @Nullable State getState(HomekitCharacteristicType characteristic) {
    ...
    if (taggedItem.isPresent()) {
      return taggedItem.get().getVariable().getValue();
    }
    logger.debug(
        "State for characteristic {} at accessory {} cannot be retrieved.",
        characteristic,
        accessory.getName());
    return null;
  }*/

  /*protected @Nullable <T extends State> T getStateAs(
      HomekitCharacteristicType characteristic, Class<T> type) {
 ...
 /* protected @Nullable Double getStateAsTemperature(HomekitCharacteristicType characteristic) {
    ...
    /**
     * return configuration attached to the root accessory, e.g. groupItem. Note: result will be
     * casted to the type of the default value. The type for number is BigDecimal.
     *
  ...
  /*protected boolean getAccessoryConfigurationAsBoolean(String key, boolean defaultValue) {
    ...
    /**
     * return configuration of the characteristic item, e.g. currentTemperature. Note: result will be
     * casted to the type of the default value. The type for number is BigDecimal.
     *
     ...
     * @return configuration value
     */
  /*protected <T> T getAccessoryConfiguration(
      HomekitCharacteristicType characteristicType, String key, T defaultValue) {
    return getCharacteristic(characteristicType)
        .map(homekitTaggedItem -> homekitTaggedItem.getConfiguration(key, defaultValue))
        .orElse(defaultValue);
  }*/
  ...
  protected <T extends Enum<T> & CharacteristicEnum> Map<T, Object> createMapping(
      HomekitCharacteristicType characteristicType,
      Class<T> klazz,
     ...
     * @param inverted if ON/OFF and OPEN/CLOSED should be inverted by default (inverted on the item
     *     will double-invert)
  ...
  /*protected <T extends Enum<T> & CharacteristicEnum> Map<T, Object> createMapping(
    ...
    /**
     * takes item state as value and retrieves the key for that value from mapping. e.g. used to map
     * StringItem value to HomeKit Enum
     *
     ...
     */
  /*public <T> T getKeyFromMapping(
      HomekitCharacteristicType characteristicType, Map<T, Object> mapping, T defaultValue) {
    final Optional<HomekitTaggedItem> c = getCharacteristic(characteristicType);
    if (c.isPresent()) {
      return HomekitCharacteristicFactory.getKeyFromMapping(
          c.get(), c.get().getVariable().getValue(), mapping, defaultValue);
    }
    return defaultValue;
  }*/
  ...
  /*protected void addCharacteristic(HomekitTaggedItem item, Characteristic characteristic)
    ...
    /**
     * If the primary service does not yet exist, it won't be added to it. It's the resposibility of
     * the caller to add characteristics when the primary service is created.
     *
  ...
  /*public void addCharacteristic(Characteristic characteristic)
    ...
    if (rawCharacteristics.containsKey(characteristic.getClass())) {
      logger.warn(
          "Accessory {} already has a characteristic of type {}; ignoring additional definition.",
          accessory.getName(),
    ...
    /**
     * Takes the NameCharacteristic that normally exists on the AccessoryInformationService, and puts
     * it on the primary service.
     */
    public void promoteNameCharacteristic() {
        ...
        if (service != null) {
            try {
                ...
                service
                        .getClass()
                        .getMethod("addOptionalCharacteristic", NameCharacteristic.class)
                        .invoke(service, characteristic.get());
                ...
                // addOptionalCharacteristic(NameCharacteristic),
  ...
  /*protected BooleanItemReader createBooleanReader(
    ...
    return new BooleanItemReader(taggedItem.getVariable(), trueOnOffValue);
  }*/

  /*protected BooleanItemReader createBooleanReader(HomekitCharacteristicType characteristicType) {
    final HomekitTaggedItem taggedItem = getCharacteristic(characteristicType)
            .orElseThrow(() -> new IncompleteAccessoryException(characteristicType));
    return new BooleanItemReader(taggedItem.getVariable(), OnOffType.of(!taggedItem.isInverted()));
  }*/

  /*protected BooleanItemReader createBooleanReader(
      HomekitCharacteristicType characteristicType,
      BigDecimal trueThreshold,
  ...
  /*protected BooleanItemReader createBooleanReader(HomekitCharacteristicType characteristicType) {
    ...
    /**
     * Calculates a string as json of the configuration for this accessory, suitable for seeing if the
     * structure has changed, and building a dummy accessory for it. It is _not_ suitable for actual
     * publishing to by HAP-Java to iOS devices, since all the IIDs will be set to 0. The IIDs will
     * get replaced by actual values by HAP-Java inside of DummyHomekitCharacteristic.
     */
