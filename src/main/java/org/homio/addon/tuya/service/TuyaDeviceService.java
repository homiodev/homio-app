...
public class TuyaDeviceService implements ServiceInstance<TuyaDeviceEntity>, DeviceInfoSubscriber, DeviceStatusListener {
    ...
    public TuyaDeviceService(EntityContext entityContext, TuyaDeviceEntity entity/*,
        @Nullable List<SchemaDp> schemaDps, Gson gson,
    ...
    @Override
    public boolean entityUpdated(@NotNull TuyaDeviceEntity entity) {
        this.entity = entity;
        this.initialize();
        return false;
    }
    ...
    private void processChannelStatus(Integer dp, Object rawValue) {
        ...
        if (property != null) {
            ...
            List<TuyaDeviceProperty> channelIds = properties.values().stream().filter(p -> p.getDp2() == dp).toList();
            if (channelIds.isEmpty()) {
                log.debug("Could not find channel for dp '{}' in thing '{}'", dp, entity.getTitle());
            } else {
                if (Boolean.class.isAssignableFrom(rawValue.getClass())) {
                    for (TuyaDeviceProperty channelId : channelIds) {
                        channelId.getStateHandler().apply(rawValue);
                    }
                    return;
                }
                log.warn("Could not update channel '{}' with value {}. Datatype incompatible.",
                    channelIds, rawValue);
            }
        }
    }
        ...
        if (status) {
            ...
            if (tuyaDevice != null && pollingInterval > 0) {
                pollingJob = entityContext.bgp().builder("tuya-device-pull-%s".formatted(entity.getEntityID()))
                                          ...
                                          .execute(tuyaDevice::refreshStatus);
            }
        } else {
    ...
    public void initialize() {
        ...
        if (properties == null || properties.isEmpty()) {
            ...
            if (!schemaDps.isEmpty()) {
                ...
                addChannels(schemaDps);
            } else {
                entity.setStatus(Status.ERROR, "No channels added and schema not found");
        ...
        if (!entity.getIp().isBlank()) {
            ...
            entity.setStatus(Status.INITIALIZE, "Waiting for IP address");
            udpDiscoveryListener.registerListener(entity.getDeviceID(), this);
        }
    ...
    public void deviceInfoChanged(DeviceInfo deviceInfo) {
        ...
        this.tuyaDevice = new TuyaDevice(gson, this, eventLoopGroup, entity.getDeviceID(),
            entity.getLocalKey().getBytes(UTF_8), deviceInfo.ip(), deviceInfo.protocolVersion());
    }

    private void addChannels(List<SchemaDp> schemaDps) {
       ...
       /* Map<String, Channel> channels = new HashMap<>(schema.entrySet().stream().map(e -> {
        ...
        channelSuffixes.forEach(suffix -> switchChannels.forEach(channel -> {
            TuyaDeviceProperty switchChannel = properties.get(channel + suffix);
            if (switchChannel != null) {
                // remove switch channel if brightness or color is present and add to dp2 instead
                TuyaDeviceProperty colourChannel = properties.get("colour_data" + suffix);
                TuyaDeviceProperty brightChannel = properties.get("bright_value" + suffix);
                boolean remove = false;

                if (colourChannel != null) {
                    colourChannel.setDp2(switchChannel.getDp());
                    remove = true;
                }
                if (brightChannel != null) {
                    brightChannel.setDp2(switchChannel.getDp());
                    remove = true;
                }

                if (remove) {
                    properties.remove(channel + suffix);
                }
            }
        }));
    }
