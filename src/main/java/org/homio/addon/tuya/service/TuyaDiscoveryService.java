...
public class TuyaDiscoveryService implements ItemDiscoverySupport {
    ...
    @SneakyThrows
    private void processDeviceResponse(List<DeviceListInfo> deviceList,
        ...
        if (page == 0 || deviceList.size() == 100) {
            int nextPage = page + 1;
            List<DeviceListInfo> nextDeviceList = tuyaProjectService.getAllDevices(nextPage).get();
            processDeviceResponse(nextDeviceList, tuyaProjectService, nextPage, result);
        }
    }

    @SneakyThrows
    private void processDevice(DeviceListInfo device, TuyaOpenAPI api, DeviceScannerResult result) {
        List<FactoryInformation> infoList = api.getFactoryInformation(List.of(device.id)).get();
        ...
        if (entity == null) {
            ...
            DeviceSchema schema = api.getDeviceSchema(device.id).get();
